...
class cls_image
{
    ...
    function upload_image($upload, $dir = '', $img_name = '')
    {
        ...
        if (!file_exists($dir))
        {
            ...
            {
                /* 创建目录失败 */
                $this->error_msg = sprintf("s%目录属性为只读", $dir);
        ...
        {
            $this->error_msg = sprintf("s%文件上传失败", $upload['name']);
    ...
    function make_thumb($img, $thumb_width = 0, $thumb_height = 0, $path = '', $bgcolor='')
    {
        ...
        if (!$org_info)
        {
            $this->error_msg = sprintf("s%的原始文件信息丢失", $img);
        ...
        {
            $this->error_msg = sprintf("s%是不支持的文件类型", $this->type_maping[$org_info[2]]);
        ...
        if (!file_exists($dir))
        {
            ...
            {
                /* 创建目录失败 */
                $this->error_msg  = sprintf("s%这个文件属性为只读", $dir);
                $this->error_no   = ERR_DIRECTORY_READONLY;
                return false;
            }
        }
