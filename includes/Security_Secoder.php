 ...
 * 安全验证码
 * 
 * 安全的验证码：验证码文字扭曲、旋转，使用不同字体，添加干扰码。
...
class Security_Secoder {
	...
	public static $codeSet_length; 
	public static $fontSize = 21;     // 验证码字体大小(px)
	...
	public static function entry() {
		self::$codeSet_length = strlen(self::$codeSet);
		// 图片宽(px)
		...
		// 验证码使用随机字体 
		$ttf = dirname(__FILE__) . '/ttfs/' . mt_rand(4, 4) . '.ttf';  
		if (self::$useNoise) {
			// 绘杂点
			self::_writeNoise();
		} 
		...
		for ($i = 0; $i<self::$length; $i++) {
			$code[$i] = self::$codeSet[mt_rand(0, self::$codeSet_length)];
		...
		for($i = 0; $i < 10; $i++){
			...
			for($j = 0; $j < 5; $j++) {
			    ...
			    imagestring(
			        ...
			        self::$codeSet[mt_rand(0, self::$codeSet_length)], // 杂点文本为随机的字母或数字
			        $noiseColor
			    );
			}
		}
...
// useage
/*
Security_Secoder::$useNoise = false;  // 要更安全的话改成true
Security_Secoder::$useCurve = true;
Security_Secoder::entry();
*/

/*
// 验证验证码
if (!Security_Secoder::check(@$_POST['secode'])) {
	print 'error secode';
}
*/
?>